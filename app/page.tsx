...
export default function PurchaseOrderPage() {
  ...
  const [deliveryAddon, setDeliveryAddon] = useState<AddonWithSelection | null>(null)
    ...
    const fetchData = async () => {
      try {
        ...
        if (productsData.success) {
          const productsWithQuantity: ProductWithQuantity[] = productsData.data.map((product: Product) => ({
            ...product,
            quantity: 0,
            icon: getIconForType(product.type),
            features_array: typeof product.features === 'string' ? JSON.parse(product.features) : (product.features || [])
          }))
        ...
        if (addonsData.success) {
          const addonsWithSelection: AddonWithSelection[] = addonsData.data.map((addon: Addon) => ({
            ...addon,
            selected: false
          }))
          ...
          // Find delivery addon
          const delivery = addonsWithSelection.find(addon => addon.name === 'Delivery & Disposal')
          if (delivery) {
  ...
  const updateQuantity = (productId: string, newQuantity: number) => {
    setProducts((prev) =>
      prev.map((product) => (product.id === productId ? { ...product, quantity: Math.max(0, newQuantity) } : product)),
    )
  }

  const calculateTotal = () => {
    const productTotal = products.reduce((sum, product) => sum + product.price_per_bundle * product.quantity, 0)
      ...
      <div className="h-fit border-y px-4 md:px-15 border-[rgba(217,231,247,1)]">
        <div className="w-full h-fit px-3 py-2 border-x bg-linear-to-r from-transparent via-blue-100/40 to-transparent flex justify-center items-center border-[rgba(217,231,247,1)]">
          <p className="text-sm text-center text-[rgba(5,81,207,1)]">
            All orders have a 4 day lead time. Rush orders incur an additional fee.
          </p>
        </div>
      </div>
      ...
      <div className="w-full px-4 md:px-15 border-b-[rgba(217,231,247,1)] border-solid bg-[repeating-linear-gradient(315deg,rgba(217,231,247,1)_0_1px,transparent_1px_6px)]">
        ...
        <div className="flex w-full px-3 py-10 border border-x border-[rgba(217,231,247,1)] bg-[rgba(248,248,249,1)] flex-col items-center justify-start border-t-0 border-b-0 border-[rgba(217,231,247,1)] gap-6">
          <div className="flex-column w-full max-w-125 h-fit px-4 space-y-2.5">
            <p className="text-xl/4 text-center text-[rgba(77,84,97,1)] font-semibold">Purchase Order</p>
            <p className="text-sm/3 text-center text-[rgba(77,84,97,1)] text-base">August 27, 2024</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-125 space-y-4">
                ...
                >
                  <div className="flex w-full px-1 justify-between">
                    <div className="flex gap-1.5">
                      ...
                      <p className="text-sm font-medium text-[rgba(77,84,97,1)] self-center">{product.type}</p>{" "}
                    </div>
                    <div className="flex w-fit space-x-0.5">
                      ...
                      <p className="text-sm font-medium font-sans text-[rgba(5,81,207,1)] self-center">/bundle</p>
                    </div>
                  </div>
                  <div className="px-3 py-4 space-y-2 bg-blue-50/60 rounded-md border border-[rgba(217,231,247,1)]">
                    <h3 className="text-lg/4 text-[rgba(5,81,207,1)] font-medium">{product.name}</h3>
                    <p className="mt-1 text-sm/3 text-[rgba(5,81,207,1)]">{product.description}</p>
                  </div>

                  <div className="grid grid-cols-2 gap-y-2 px-0 py-1.5 md:px-1">
                    {product.features_array.map((feature, index) => (
                      <div key={index} className="flex gap-x-1 md:gap-x-2 w-fit md:max-w-none">
                        <Check className="w-4 h-4 stroke-[1.5] md:w-5 md:h-5 md:stroke-[1.75] text-[rgba(77,84,97,1)] self-center" />
                        <span className="text-[rgba(77,84,97,1)] text-xs md:text-sm self-center">{feature}</span>
                      </div>
                    ))}
                  </div>
            ...
            <div className="flex-col w-full max-w-125 p-4 md:p-6 border border-[rgba(217,231,247,1)] bg-[rgba(250,250,250,0.8)] rounded-lg gap-2.5 space-y-5">
              <div className="inline-flex space-x-1.5">
                ...
                <h3 className="text-left text-lg font-medium text-[rgba(157,187,235,1)]">Contact Information</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  <label className="text-sm font-medium px-px text-[rgba(77,84,97,1)]">State</label>
                  <SelectScrollable
                    value={selectedState}
                    onValueChange={(value) => {
                ...
                <FormField
                  ...
                  onChange={(e) => setContactInfo((prev) => ({ ...prev, poNumber: e.target.value }))}
                />
                ...
                <div className="md:col-span-2">
                  ...
                  <DateTimePicker date={fulfillmentDate} onDateChange={setFulfillmentDate} />
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setContactInfo((prev) => ({ ...prev, specialInstructions: e.target.value }))}
            ...
            <div className="flex flex-col max-w-125 p-4 md:p-6 py-6 space-y-5 border border-green-200 rounded-lg items-center bg-linear-to-b from-[#B7FFE6] to-[#B7FFE6]/0">
              <div className="flex flex-col space-y-2 items-start">
                ...
                Full service delivery and pulp disposal. Help reduce waste in landfills & contribute to enriching farmlands.
                </p>
              </div>
              <div className="flex-col w-full space-y-1">
                <button
                  ...
                  onClick={() => deliveryAddon && setDeliveryAddon({ ...deliveryAddon, selected: !deliveryAddon.selected })}
                ...
                >
                  ...
                  >
                    {deliveryAddon?.selected ? "Remove" : isDeliveryHovered ? "Add to Order" : `$${deliveryAddon?.price || 0} /order`}
                  </span>
                </button>
            ...
            <div ref={orderSummaryRef} className="flex-col w-full max-w-125 p-4 md:p-6 border border-[rgba(217,231,247,1)] bg-[rgba(250,250,250,0.8)] rounded-lg gap-2.5">
              <h3 className="px-1 font-semibold text-2xl tracking-tight text-[rgba(77,84,97,1)] self-center">
                Order Summary
              </h3>
              <div className="flex-col px-1 py-2">
                <div className="flex justify-between py-4">
                  <span className="font-medium text-[rgba(127,138,148,1)] text-sm">Item</span>
                  <span className="font-medium text-[rgba(127,138,148,1)] text-sm">Cost</span>
                </div>
                <div className="border-b border-gray-200">
                </div>
                {(() => {
                  const visibleProducts = products.filter(product => product.quantity > 0);
                    ...
                    <>
                        ...
                        <div key={product.id} className="">
                          <div className="flex justify-between py-4">
                            <div className="flex flex-col">
                              <span className="w-fit font-medium text-sm text-[rgba(77,84,97,1)]">{product.type}</span>
                              <span className="text-xs text-[rgba(127,138,148,1)] self-center">
                                {product.quantity} bundle{product.quantity !== 1 ? "s" : ""} @ ${product.price_per_bundle}
                              </span>
                            </div>
                          ...
                          {index < visibleProducts.length - 1 || hasDelivery ? (
                      ...
                      {deliveryAddon?.selected && (
                        <div className="space-y-1">
                          <div className="flex justify-between py-4">
                            <div className="flex flex-col">
                              <span className="w-fit font-medium text-sm text-[rgba(77,84,97,1)]">Delivery & Disposal</span>
                              {fulfillmentDate && (
                                <span className="text-xs text-[rgba(127,138,148,1)]">
                                  ...
                                  @{" "}
                                  {fulfillmentDate.toLocaleTimeString("en-US", {
                                    hour: "numeric",
                                    minute: "2-digit",
                                    hour12: true,
                                  })}
                            ...
                            <span className="font-medium text-[rgba(77,84,97,1)] text-sm self-center">${deliveryAddon?.price}</span>
                          </div>
              ...
              <div className="">
                <div className="flex justify-between px-1 py-2">
                  <span className="text-[rgba(77,84,97,1)] font-semibold text-sm">Order Total</span>
                  <span className="font-semibold text-[rgba(77,84,97,1)] text-sm tracking-tight">${calculateTotal().toFixed(2)}</span>
                </div>
                ...
                <p className="text-xs text-center px-2 py-2 text-[rgba(127,138,148,1)]">
                  This is a purchase order submission. You will receive an email upon approval.
                </p>
              </div>
            </div>
